...
@section('title_db', 'Edit Gejala')
@section('content_dashboard')
<section class="w-full h-full">
    <p class="text-lg uppercase">Edit Data Gejala</p>
    <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark p-8 max-w-132.5 mt-3">
        <form action="{{ Route('update.gejala', ['gejalaId' => $gejala->id]) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-4.5">
                ...
                @enderror
                <input type="text" name="kode_gejala" value="{{ old('kode_gejala', $gejala->kode_gejala) }}"
            ...
            <div class="mb-4.5">
                ...
                <span class="text-xs text-meta-7/70 mb-1.5 capitalize" role="alert">{{ $message }}</span>
                @enderror
                <textarea name="nama_gejala" value="{{ old('nama_gejala', $gejala->nama_gejala) }}"
                    placeholder="Masukkan deskripsi gejala..."
                    class="w-full rounded border-[1.5px] {{ $errors->has('nama_gejala') ? 'border-danger' : 'border-stroke' }} bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary">{{ $gejala->nama_gejala }}</textarea>
            </div>
            <div class="mb-4.5">
                ...
                <div class="relative z-20 bg-transparent dark:bg-form-input">
                    <select name="penyakit_id" class="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary">
                        @foreach ($penyakit as $p)
                          <option value="{{ $p->id }}" @if ($p->id === $gejala->penyakit_id) {{ 'selected' }} @endif>{{ $p->nama_penyakit }}</option>
                        @endforeach
                    </select>
