...
<section class="w-full h-full">
    <p class="text-lg uppercase">Tambah Data Penyakit</p>
    <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark p-8 max-w-132.5 mt-3">
        <form action="{{ Route('create.penyakit') }}" method="POST" x-data="{ isSubmitting: false }" @submit="isSubmitting = true">
            @csrf
            <div class="mb-4.5">
            ...
            <div class="mb-4.5">
                ...
                <textarea type="text" name="deskripsi" value="{{ old('deskripsi') }}"
                    ...
                    class="w-full rounded border-[1.5px] {{ $errors->has('deskripsi') ? 'border-danger' : 'border-stroke' }} bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"></textarea>
            </div>
            <button type="submit" :disabled="isSubmitting" class="disabled:cursor-not-allowed mt-4 flex w-full justify-center rounded-md bg-primary hover:bg-meta-5 p-2 font-medium text-gray" x-text="isSubmitting ? 'Processing...' : 'Submit'">
                    Submit
            </button>
        </form>
    </div>
