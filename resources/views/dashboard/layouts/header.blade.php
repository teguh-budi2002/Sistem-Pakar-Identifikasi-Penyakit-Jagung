  ...
  <div class="flex flex-grow items-center justify-between py-4 px-4 shadow-2 md:px-6 2xl:px-11">
    <div class="flex items-center gap-2 sm:gap-4 lg:hidden">
      ...
      <a class="block flex-shrink-0 lg:hidden" href="index.html">
        <img src="{{ asset('images/logo_dashboard/logo.svg') }}" alt="Logo" />
      </a>
    </div>
      ...
      <div class="relative" x-data="{ dropdownOpen: false }" @click.outside="dropdownOpen = false">
        <a class="flex items-center gap-4" href="#" @click.prevent="dropdownOpen = ! dropdownOpen">
          <span class="hidden text-right lg:block">
            <span class="block text-sm font-medium text-black dark:text-white">{{ Auth::user()->fullname }}</span>
            <span class="block text-xs font-medium">Admin Dashboard</span>
          </span>
        ...
        <div x-show="dropdownOpen"
          ...
          <form action="{{ route('logout') }}" method="POST">
            @csrf
            <button
              class="flex items-center gap-3.5 py-4 px-6 text-sm font-medium duration-300 ease-in-out hover:text-danger lg:text-base" type="submit">
