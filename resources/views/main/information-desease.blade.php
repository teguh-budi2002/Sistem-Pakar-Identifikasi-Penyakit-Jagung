...
@if (!empty($data))
  <div class="flex flex-col items-center justify-centestart mt-12">
    <img src="{{ asset('images/penyakit/' . $data['img_penyakit']) }}" class="w-auto max-h-96" alt="{{ $data['img_source'] }}">
    <p class="text-sm text-slate-600 font-semibold mt-2">Sumber: <span class="text-slate-400 font-normal">{{ $data['img_source'] }}</span></p>
  </div>
